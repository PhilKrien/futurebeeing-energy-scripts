...
LEGEND_REFURB_STATE_SOURCE = json.dumps(legend_settings["refurbishment_state"]["source"])

TECH_KEY = legend_settings["tech"]["key"]   # must exactly match the tag key in build_building_tags
LEGEND_TECH_SOURCE = json.dumps(legend_settings["tech"]["source"])
...
def get_osm_buildings(scenario_id):
    ...
    osm_gdf["geometry"] = osm_gdf["geometry"].apply(lambda g: Polygon(g["coordinates"][0]))
    ...
    if len(hp_cases) > 0:
        ...
        if len(hp_only_cases) > 0:
            ...
            for feature in hp_only_cases:
                ...
                # Profiles
                summed_elec_import_profiles += np.array(tags["electricity_import_profile"])
                summed_elec_demand_profiles += np.array(tags["electricity_demand_profile"])
 
                # Sums
                total_heat_produced += tags["ashp_production"]
                total_ashp_emissions += tags["ashp_emissions"]
                ...
                total_ashp_heating_costs_om += tags["ashp_costs_om"]
        ...
        if len(hp_pv_cases) > 0:
            ...
            for feature in hp_pv_cases:
                ...
                summed_elec_export_profiles += np.array(tags["electricity_export_profile"])
    ...
    if len(ashp_gas_cases) > 0:
        ...
        if len(ashp_gas_pv_cases) > 0:
            ...
            for feature in ashp_gas_pv_cases:
                ...
                summed_elec_import_profiles += np.array(tags["electricity_import_profile"])
                summed_elec_demand_profiles += np.array(tags["electricity_demand_profile"])
                summed_pv_production_profiles += np.array(tags["pv_generation_profile"])
                summed_elec_export_profiles += np.array(tags["electricity_export_profile"])
...
def create_init_inputs(tagged_features):
    ...
    for input in inputs:
        ...
        if input_key in inputs_to_adapt["categories"]:
            print("In categories")
            input_default = len([feature for feature in tagged_features if feature["properties"]["tags"]["heat_technology"] == inputs_to_adapt["categories"][input_key]])
            ...
            input_default = len([feature for feature in tagged_features if feature["properties"]["tags"][inputs_to_adapt["bools"][input_key]] == True])
            input["default"] = input_default
            input["max"] = num_tagged_features
            input_patches.append(input)
        else: 
...
def process_new_scenario(scenario_id):
    ...
    elec_demand_legend = build_demand_legend(elec_demand, "Electricity demand")
    publish_legend(scenario_id, existing_stats, "elec_cluster", elec_demand_legend)
