...
def update_heat_techs(tagged_features, inputs):
    """Redistributes buildings across heating technologies according to the
    hp/district-heat/pv scenario-input settings (percentages of the eligible pool, plus
    a set_ashp_gas flag -- gas is simply whatever's left over), then applies the matching
    pv_activated redistribution.

    Args:
        tagged_features: List of GeoJSON features with populated properties.tags.
        inputs: Dict of this scenario's current input values, as returned by fetch_inputs
            (hp_setting, dh_setting, pv_setting as percentages, set_ashp_gas as bool).
    ...
    if len(remaining_candidates) > 0:
        # Reset the rest back to whatever the data originally set
        for feature in tagged_features:
