"""Poller-run variant of the refurbishment-state pipeline: on a SCENARIO_CHANGED or
MANUAL event, lowers the event's scenario's building refurbishment states to its
min_refurb_state input, recomputes heating/PV energy statistics via OEP system
imports, and publishes the updated building tags, heat-demand legend and energy
stats. Reads its API key and OEP/legend config from the poller's environment and
the /v1/scripts/files endpoint rather than local files -- see change_refurb_state.py
for the local/standalone variant.
"""
...
if __name__ == "__main__":
    ...
    if event_type == "SCENARIO_CHANGED":
